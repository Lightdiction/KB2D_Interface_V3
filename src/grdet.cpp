...
{
    kbDev.sendCom(MIDI_MINPOS + (char)(qPow(value - 1, POW_MINPOS)));
    int result = kbDev.checkFeedback(Check_MinPos);
    if (result < 0)
        SendError(this, tr("No Feedback / Error on Feedback received."), GrDet_DetSpeedSlider);
    else
    {
        ui->minTimeSpinBox->setValue(qCeil(qPow(result, 1 / POW_MINPOS)) + 1);
        ui->minTimeSlider->setValue(qCeil(qPow(result, 1 / POW_MINPOS)) + 1);
    }
}

void MainWindow::on_maxTimeSlider_valueChanged(int value)
{
    kbDev.sendCom(MIDI_MAXPOS + (char)(qPow(value - 1, POW_MAXPOS)));
    ...
    {
        ui->maxTimeSpinBox->setValue(qCeil(qPow(result, 1 / POW_MAXPOS)) + 1);
        ui->maxTimeSlider->setValue(qCeil(qPow(result, 1 / POW_MAXPOS)) + 1);
    }
}
