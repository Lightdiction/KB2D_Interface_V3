...
int ComHwKb2d::checkFeedback(unsigned char commIndex)
{
    ...
    while (kbDev.isWaitingFb())
    {
        ...
        {
            // Communication error...
            SendError(0, QObject::tr("Timeout - Device is Busy"), ComHw_CheckFeedback_1);
    ...
    if (kbDev.isConnected() == 1)
    {
        for (int tryN = 0; tryN < 3; tryN++)
        {
            ...
            while (kbDev.isWaitingFb())
            {
                ...
                if (timer.elapsed() > 500)
                {
                    ...
                    {
                        // Communication error...
                        SendError(0, QObject::tr("Timeout - No communication with Device"), ComHw_CheckFeedback_2);
                        emit comFailed();
                        kbDev.waitForFeedback(0);
                        return -1;
                    }
        ...
        catchPar2 = harpIn.getParam(1);
        catchPar3 = harpIn.getParam(2);
        SendError(0, QObject::tr("Incorrect Communication with Device"), ComHw_CheckFeedback_3);
    }
