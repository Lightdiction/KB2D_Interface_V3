...
#ifdef WIN_PLATFORM
#define NAME_KB_RES_OUT     "KB2D Interactive"
#define NAME_KB_RES_IN      "MIDIIN2 (KB2D Interactive)"
#define NAME_FW_RES_OUT     "LD Firmware Updater"
#define NAME_FW_RES_IN      "LD Firmware Updater"
#endif
#ifdef LIN_PLATFORM
#define NAME_KB_RES_OUT     "KB2D Interactive MIDI 1"
#define NAME_KB_RES_IN      "KB2D Interactive MIDI 2"
#define NAME_FW_RES_OUT     "LD Firmware Updater MIDI 1"
#define NAME_FW_RES_IN      "LD Firmware Updater MIDI 1"
#endif
#ifdef MAC_PLATFORM
#define NAME_KB_RES_OUT     "KB2D Interactive Port 1"
#define NAME_KB_RES_IN      "KB2D Interactive Port 2"
#define NAME_FW_RES_OUT     "LD Firmware Updater Port 1"
#define NAME_FW_RES_IN      "LD Firmware Updater Port 1"
#endif
...
void MainWindow::resetMidiPorts()
{
    ...
    ui->SNButton->setVisible(testMode);
...
void MainWindow::testConnectedDevices()
{
    ...
    if (connectionReq)
    {
        ...
        {
            if ((saveNameMidi == NAME_FW_RES_OUT) && (saveNameMidiIn == NAME_FW_RES_IN))
            {
                kbDev.setConnected(0);
            ...
            else
            {
                ...
                else if (tmpResUp == -1)    // Force DFU if firmware is obsolete
                {
                    ...
                    QTimer::singleShot(0, this, [this] () { this->retryConnection(5); });
                    return;
                }
                else
                {
        ...
        else if (((globQSettings.value("KB In", "").toString() != "") && (globQSettings.value("KB In", "").toString() != NAME_KB_RES_IN) && \
                  !ui->midiInComboBox->currentIndex()) || \
                 ((globQSettings.value("KB Out", "").toString() != "") && (globQSettings.value("KB Out", "").toString() != NAME_KB_RES_OUT) && \
